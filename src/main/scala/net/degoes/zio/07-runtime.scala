...
object CustomRuntime {
  ...
  def myZIOMethod(): ZIO[Any, Nothing, Unit] =
    for {
   ...
   * environment.
   */
  lazy val customRuntime = Runtime(defaultEnvironment ++ ZEnvironment(AppConfig("My App")), defaultFiberRefs, defaultRuntimeFlags)
